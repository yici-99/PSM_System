...
@foreach($deta as $detaa)
  ...
  <td>{{$detaa->supervisorID}}</td>
    ...
    <td>{{$detaa->svemail}}</td>
    <td><a href="/viewsvprofile/{{ $detaa->studentID }}"><button type="button" style="background-color: white; border: 1px solid white;" > <i class="fas fa-eye"></i> </button></a></td>
    <td><a href="/searchsvlist/{{ $detaa->studentID }}"><button type="button" style="background-color: white; border: 1px solid white;" > <i class="fas fa-trash"></i> </button></a></td>
  </tr>
  @endforeach
</tbody>
      </table>
