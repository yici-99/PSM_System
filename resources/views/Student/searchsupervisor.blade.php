...
<br><br><br><br><br><br>
<div class=search2>
                <form action="/searchsupervisor/search" method="GET" role="search">
                    ...
                    &emsp;&emsp;<button class="btn btn-info" style="width:10%;height:10%;font-size:18px;background-colour:white" size="5" type="submit" title="Search projects"><i class="fas fa-search"></i></button>
                        ...
                        <a href="/searchsupervisor" class=" mt-1">&emsp;<button  class="btn btn-danger" style="width:10%;height:10%;font-size:18px;" size="5" type="button" title="Refresh page"><i class="fas fa-sync-alt"></i></button></a>
                    </div>
                    @if(!empty($successMsg))
...
@foreach($deta as $detaa)
  ...
  <td>{{$detaa->supervisorID}}</td>
    ...
    <td>{{$detaa->svemail}}</td>
    <td><a href="/svprofile/{{ $detaa->supervisorID }}"><button type="button" style="background-color: white; border: 1px solid white;" > <i class="fas fa-eye"></i> </button></a></td> </tr>
  @endforeach
</tbody>
      </table>
<br><br>
